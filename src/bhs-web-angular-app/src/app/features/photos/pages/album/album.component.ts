...
export class AlbumComponent {
  ...
  vm$ = this.activatedRoute.paramMap.pipe(
    ...
    map(album => ({ album, isLoading: false, error: undefined })),
    startWith({ album: undefined, isLoading: true, error: undefined }),
    catchError((error: unknown) => {
      const message = parseErrorMessage(error) ?? 'An unknown error occurred.';
      return of({ album: undefined, isLoading: false, error: message });
    }),
  );
}
